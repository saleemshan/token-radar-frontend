...
const FastNewsPanel = () => {
    ...
    useEffect(() => {
        if (refetchSignal !== lastHandledSignal.current) {
            ...
            if (process.env.NEXT_PUBLIC_NODE_ENV !== 'production') console.log('Refetch | Idle for too long, refetching Fast News data')
            handleFetchNews(1)
        ...
        <div className="max-h-full h-full overflow-y-auto divide-y divide-border flex flex-col flex-1 relative focus:outline-none" ref={containerRef}>
            {initialFetched ? (
                <>
                    {news && news.length > 0 ? (
                        news.map((data, index) => {
                            return <TradFiItem key={`${data.id}-${index}`} data={data} headlineLink={true} />
                        })
                    ) : (
                        <>{!loading && <EmptyData />}</>
                    )}
