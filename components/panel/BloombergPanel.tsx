...
const BloombergPanel = () => {
    ...
    const handleFetchNews = useCallback(
        async (pageNum: number) => {
            ...
            try {
                ...
                const response = await axios.get(`/api/news-trading/tradfi/bloomberg`, {
                    params: {
                        page: pageNum,
                        limit: 20,
                    },
    ...
    useEffect(() => {
        if (refetchSignal !== lastHandledSignal.current) {
            ...
            if (process.env.NEXT_PUBLIC_NODE_ENV !== 'production') console.log('Refetch | Idle for too long, refetching Bloomberg data')
            handleFetchNews(1)
    ...
    return (
        <div className="max-h-full h-full overflow-y-auto divide-y divide-border flex flex-col flex-1  relative" ref={containerRef}>
            {initialFetched ? (
                <>
                    {news && news.length > 0 ? (
                        news.map((data, index) => {
                            return <TradFiItem key={`${data.id}-${index}`} data={data} headlineLink={true} showExternalUrlIcon={false} />
                        })
                    ) : (
                        <>{!loading && <EmptyData />}</>
                    )}
