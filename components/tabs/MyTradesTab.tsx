...
const MyTradesTab = ({ tokenAddress, chain, tokenData }: { tokenAddress: string; chain: string; tokenData: Token | undefined }) => {
    ...
    const { data, isLoading } = useTokenMyPositionsData(chain, tokenAddress, userPublicWalletAddresses[chain === 'solana' ? 'solana' : 'ethereum'])

    return (
        <div className="flex flex-col min-h-[60vh] max-h-[60vh] relative">
            {ready && authenticated ? (
                <div className="w-full bg-black max-h-full overflow-hidden min-h-full flex flex-col">
                    <div className="flex p-3 items-center justify-between border-b border-border">
                        <div className="text-sm font-semibold leading-6 text-white ">{tokenData?.symbol} Trades</div>
                    </div>
                                ...
                                <div className="flex flex-col items-center justify-center h-10">
                                    {isLoading ? (
                                        ...
                                        <div className=" text-sm text-neutral-text ">
                                            {getReadableNumber(data?.analytics?.balance_usd, 2, '$', true)}
                                        </div>
                                    )}
                                </div>
                            </div>
                    ...
                    <div
                        ...
                        className="  overflow-x-auto flex-1 mobile-no-scrollbar overflow-y-auto  border-t border-border pb-[13.5rem] md:pb-[9rem]"
                    >
                                                ...
                                                >
                                                    ...
                                                    <td
                                                        className={` text-center h-12 ${
                                                            transaction.type === 'buy' ? 'text-positive' : 'text-negative'
                                                        }`}
                                                    >
