...
export const GET = async () => {
    ...
    try {
        const response = await axios.get(`${process.env.BACKEND_API_URL}/feeds-tradfi/social/accounts-preferences`, {
            params: {
                privyId: user.userId,
            },
            headers: {
...
export const POST = async (request: NextRequest) => {
    ...
    try {
        ...
        const response = await axios.post(
            `${process.env.BACKEND_API_URL}/feeds-tradfi/social/accounts-preferences`,
            {
                privyId: user.userId,
