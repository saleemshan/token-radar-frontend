...
export class AlphaFeedPreferencesError extends Error {
    type: string
...
export const getData = async (): Promise<AlphaFeedtPreferencesBackend> => {
    const res = await axiosLib.get(`/api/alpha-feed`, {
        headers: {
...
export const useAlphaFeedPreferencesData = () => {
    const { ready, authenticated, user } = usePrivy()
    return useQuery({
        queryKey: ['alphaFeedPreferences', user?.id],
...
const updateData = async (params: AlphaFeedPreferences) => {
    const response = await axiosLib.post(`/api/alpha-feed`, params, {
        headers: {
            'Content-Type': 'application/json',
        },
    })
    ...
    if (response.data.code !== 0) {
        ...
        throw new AlphaFeedPreferencesError(
            errorMessage,
            type,
            response.data,
...
export const useMutateAlphaFeedPreferencesData = () => {
    const queryClient = useQueryClient()
    const { user } = usePrivy()
    return useMutation({
        mutationFn: (params: AlphaFeedPreferences) => updateData(params),
        onSuccess: () => {
